...
BEGIN { use_ok( 'OpenVZ::vzctl', 'quotaon' ) }
...
throws_ok { quotaon( ctid => 'invalid_ctid' ) } $invalid_ctid_rx, 'caught invalid ctid';
throws_ok { quotaon( ctid => 'invalid_name' ) } $invalid_name_rx, 'caught invalid name';

# Test bad global flag
throws_ok { quotaon( ctid => $test, flag => 'badflag' ) } $badflag_rx, 'caught bad global flag';
...
for my $flag ( keys %global_flag ) {
  ...
  my @response = quotaon( $global_flag{ $flag } );

  my $expected_cmd = sprintf 'vzctl %squotaon %s', ($flag?"--$flag ":''), $ctid;
...
throws_ok { quotaon( ctid => $test, badparm => 'blech' ) } $badparm_rx, 'caught bad parm';
