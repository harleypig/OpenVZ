...
BEGIN { use_ok( 'OpenVZ::vzctl', 'status' ) }
...
throws_ok { status( ctid => 'invalid_ctid' ) } $invalid_ctid_rx, 'caught invalid ctid';
throws_ok { status( ctid => 'invalid_name' ) } $invalid_name_rx, 'caught invalid name';

# Test bad global flag
throws_ok { status( ctid => $test, flag => 'badflag' ) } $badflag_rx, 'caught bad global flag';
...
for my $flag ( keys %global_flag ) {
  ...
  my @response = status( $global_flag{ $flag } );

  my $expected_cmd = sprintf 'vzctl %sstatus %s', ($flag?"--$flag ":''), $ctid;
...
throws_ok { status( ctid => $test, badparm => 'blech' ) } $badparm_rx, 'caught bad parm';
