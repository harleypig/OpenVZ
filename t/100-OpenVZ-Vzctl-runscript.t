...
BEGIN {
    ...
    $subcommand = 'runscript';            # <<<--- Change this to match the command you are testing against.

    use_ok( 'OpenVZ::Vzctl', qw( vzctl known_options subcommand_specs ), $subcommand );

}

my @parms = qw( script );                 # <<<--- Change this to match the parameters you are expecting (checked against
                                          # known_options).
...
my $expected_spec = {                     # <<<--- Change this to match expected hash from subcommand_specs
    ...
    script => { type => SCALAR | ARRAYREF, callbacks => { 'do not want empty values' => ignore() }, },
