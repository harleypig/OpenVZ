...
BEGIN { use_ok( 'OpenVZ::vzctl', 'mount' ) }
...
throws_ok { mount( ctid => 'invalid_ctid' ) } $invalid_ctid_rx, 'caught invalid ctid';
throws_ok { mount( ctid => 'invalid_name' ) } $invalid_name_rx, 'caught invalid name';

# Test bad global flag
throws_ok { mount( ctid => $test, flag => 'badflag' ) } $badflag_rx, 'caught bad global flag';
...
for my $flag ( keys %global_flag ) {
  ...
  my @response = mount( $global_flag{ $flag } );

  my $expected_cmd = sprintf 'vzctl %smount %s', ($flag?"--$flag ":''), $ctid;
...
throws_ok { mount( ctid => $test, badparm => 'blech' ) } $badparm_rx, 'caught bad parm';
