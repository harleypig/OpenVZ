...
BEGIN { use_ok( 'OpenVZ::vzctl', 'restart' ) }
...
throws_ok { restart( ctid => 'invalid_ctid' ) } $invalid_ctid_rx, 'caught invalid ctid';
throws_ok { restart( ctid => 'invalid_name' ) } $invalid_name_rx, 'caught invalid name';

# Test bad global flag
throws_ok { restart( ctid => $test, flag => 'badflag' ) } $badflag_rx, 'caught bad global flag';
...
for my $flag ( keys %global_flag ) {
  ...
  my @response = restart( $global_flag{ $flag } );

  my $expected_cmd = sprintf 'vzctl %srestart %s', ($flag?"--$flag ":''), $ctid;
...
throws_ok { restart( ctid => $test, badparm => 'blech' ) } $badparm_rx, 'caught bad parm';
