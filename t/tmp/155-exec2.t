...
BEGIN { use_ok( 'OpenVZ::vzctl', 'exec2' ) }
...
throws_ok { exec2( ctid => 'invalid_ctid' ) } $invalid_ctid_rx, 'caught invalid ctid';
throws_ok { exec2( ctid => $test, flag => 'badflag' ) } $badflag_rx, 'caught bad flag';
...
for my $flag ( keys %hash ) {
  ...
  my @response = exec2( $hash{ $flag } );

  my $expected_cmd = sprintf 'vzctl %sexec2 %s', ($flag?"--$flag ":''), $ctid;
...
my @response = exec2( ctid => $test, extra => 'parm' );
my $expected_cmd = "vzctl exec2 $ctid --extra parm";
