...
BEGIN { use_ok( 'OpenVZ::vzctl', 'runscript' ) }
...
throws_ok { runscript( ctid => 'invalid_ctid' ) } $invalid_ctid_rx, 'caught invalid ctid';
throws_ok { runscript( ctid => $test, flag => 'badflag' ) } $badflag_rx, 'caught bad flag';
...
for my $flag ( keys %hash ) {
  ...
  my @response = runscript( $hash{ $flag } );

  my $expected_cmd = sprintf 'vzctl %srunscript %s', ($flag?"--$flag ":''), $ctid;
...
my @response = runscript( ctid => $test, extra => 'parm' );
my $expected_cmd = "vzctl runscript $ctid --extra parm";
