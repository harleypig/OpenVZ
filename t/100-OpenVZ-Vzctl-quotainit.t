...
# Test
#   vzctl, known_options, subcommand_specs for each command
#   capabilities, iptables_modules, features for set
...
throws_ok { execute( {} ) } qr/Mandatory parameter 'command' missing in call/, 'empty hashref call to execute dies correctly (functional)';
...
throws_ok { $object->execute( {} ) } qr/Mandatory parameter 'command' missing in call/, 'empty hashref call to execute dies correctly (oop)';
...
throws_ok { vzctl( [] ) } qr/Mandatory parameters 'ctid', 'subcommand', 'flag' missing/, 'empty arrayref call to vzctl dies correctly (functional)';
throws_ok { vzctl( {} ) } qr/Mandatory parameters 'ctid', 'subcommand', 'flag' missing/, 'empty hashref call to vzctl dies correctly (functional)';

throws_ok { $object->vzctl() } qr/Expecting array or hash reference in 'spec'/, 'empty call to vzctl dies correctly (functional)';
throws_ok { $object->vzctl( '' ) } qr/Expecting array or hash reference in 'params'/, 'null call to vzctl dies correctly (functional)';
throws_ok { $object->vzctl( [] ) } qr/Mandatory parameters 'ctid', 'subcommand', 'flag' missing/, 'empty arrayref call to vzctl dies correctly (functional)';
throws_ok { $object->vzctl( {} ) } qr/Mandatory parameters 'ctid', 'subcommand', 'flag' missing/, 'empty hashref call to vzctl dies correctly (functional)';

{
    no warnings 'once';
    throws_ok { vzctl( \*GLOB ) } qr/Expecting array or hash reference in 'params'/, 'glob call to vzctl dies correctly (functional)';
    throws_ok { $object->vzctl( \*GLOB ) } qr/Expecting array or hash reference in 'params'/, 'glob call to vzctl dies correctly (functional)';
}

throws_ok { vzctl( { subcommand => 'badsubcommand' } ) } qr/did not pass regex check/, 'badsubcommand dies correctly (functional)';
throws_ok { $object->vzctl( { subcommand => 'badsubcommand' } ) } qr/did not pass regex check/, 'badsubcommand dies correctly (oop)';
